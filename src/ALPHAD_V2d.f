...
!     Version 2.0b: June 1, 2017 T.D. Johnson
!                   1. Do not report HF in report of multiple branch 
!                      alphas with no intensity.
!                   2. Fixed bug where odd-even checks from parent card
!                      not used when getting radius from HF comment.
!                   3. Fixed partial half life bug.
!     Version 2.0c  July 12, 2017 T.D. Johnson and Balraj Singh 
!                   1. Fixed issue with unplaced alpha cards.
!                   2. Do not report partial half-lives for alpha with
!                      no intensity
!     Version 2.0d: November 21, 2020 Sukhjeet Singh, Sushil Kumar and Balraj Singh
!                   1. Fixed bug pertaining to abundance 1.0 for alpha's without Intensity 
!                   2. Updated to read the official symbols for the super-heavy 
!                      elements (Z = 112-118) in an ENSDF-formatted file
!
!***************************************************************************************
!
...
!
      CHARACTER(LEN=*), PARAMETER :: version = 'ALPHAD Version 2.0d'
      CHARACTER(LEN=*), PARAMETER :: verdate = '21-Nov-2020'
...
!     (2003). More recent update to 2017Wa38)
      REAL(KIND=8), PARAMETER :: amass = 4.0026032541
!
...
!
      ...
      REAL(KIND=4), DIMENSION(i500) :: shadint
...
! Added logical here to handle unplaced alphas TDJ
      LOGICAL(KIND=4) :: seenlev
!
      CALL OPENFI
!
...
! TDJ -- check here for even-even nuclei
               ab = card(1:5)
               pamass = DVALST(ab(1:3)) + 4
               call IZEL(ab(4:5),iz)
               paze = iz + 2
               iseven = .TRUE.
               if ((DMOD(paze,2.D0)+DMOD(pamass,2.D0)).EQ.0) then                                 
                   iseven = .TRUE.
               else  
                   iseven = .FALSE.
               end if
...
!
! TDJ Added seenlev to make sure we ignore unplaced alphas
            IF(card(6:9).EQ.'  L ') THEN
               tstcom = .FALSE.
               seenlev = .TRUE.
               j = j + 1
               lacount = 0
! resetting the alpha counter
               estr = card(10:19)
               sdenl(j) = card(20:21)
               CALL LBSUP(sdenl(j))
...
! TDJ --a 'shadow intensity' to keep the fact that the intensity for 
!       this alpha is really 0
            shadint(k) = abxit(k)
            enxit(k) = enxit(j)
            denxit(k) = denxit(j)
            sdenl(k) = sdenl(j)
...
!TDJ corrected partial half-life.
            tcal(l) = t/DBLE(abxit(l))
            dtcal(l) = tcal(l)                                          &       
...
!           HF identically equal to one for even-even gs transition
            ...
            dhf(l) = 0.0
         ELSE
! TDJ this is where we could check the counts and call or not depending
! on number of branches
            if(shadint(l).NE.0 .or. n.eq.1) then
...
!---------------
     ...
     &      'HINDRANCE FACTOR'
      DO l = 1, n
         ...
         END IF
! This looks like the spot where we should put the stuff about setting halflife to blank
! Change here..TDJ 12/07/2017
         if(shadint(l).EQ.0) THEN
           outstr(4) = "     "
           doutst(4) = "  "
! Change here.. Sukhjeet Singh, Sushil Kumar, Balraj Singh 01/08/2017
           outstr(3) = "     "
           doutst(3) = "  "
           end if
   35    CALL CNVU2S(hf(l),dhf(l),outstr(5),10,doutst(5),2)
         if (hf(l).eq.0.00000) then
           outstr(5) = "     "
           doutst(2) = "  "
         end if
...
!     (2003).  Checked with 2017Wa38, and no change necessary)
